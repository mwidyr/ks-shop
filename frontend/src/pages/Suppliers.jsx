  ...
  return (
    <div className="px-4 sm:px-6 py-6 max-w-2xl">
      <div className="bg-white rounded-2xl shadow-sm p-5">
        <h2 className="font-bold text-gray-800 mb-4">Supplier</h2>
          ...
          {suppliers.map((s) => (
            <div key={s.id} className="flex items-center justify-between py-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => handleDelete(s)} className="text-xs text-red-600 hover:underline">Hapus</button>
              </div>
            </div>
          ))}
          {suppliers.length === 0 && <p className="text-sm text-gray-400 py-4">Belum ada supplier.</p>}
        </div>

        <form onSubmit={handleCreate} className="grid grid-cols-2 gap-2">
          <input placeholder="Nama Supplier *" value={form.name} onChange={(e) => setForm((s) => ({ ...s, name: e.target.value }))} required className="border border-gray-300 rounded-lg px-2 py-1.5 text-sm col-span-2" />
          <input placeholder="Nama Kontak" value={form.contact_name} onChange={(e) => setForm((s) => ({ ...s, contact_name: e.target.value }))} className="border border-gray-300 rounded-lg px-2 py-1.5 text-sm" />
          <input placeholder="Telepon" value={form.phone} onChange={(e) => setForm((s) => ({ ...s, phone: e.target.value }))} className="border border-gray-300 rounded-lg px-2 py-1.5 text-sm" />
          <input placeholder="Alamat" value={form.address} onChange={(e) => setForm((s) => ({ ...s, address: e.target.value }))} className="border border-gray-300 rounded-lg px-2 py-1.5 text-sm col-span-2" />
